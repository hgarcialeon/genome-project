...
| High | RFC-0009 Studio Milestone 1 — Governed Authoring: a Studio surface (code editor, inline validation, live Organization Graph, organization tree, ephemeral governed execution, live session event stream, deny-safe park, explicit grant, attributed approval, completion) built strictly as a projection/interaction layer over the shipped compiler targets (`compile`/`graphTarget`/`inspectTarget`/`runtimeModelTarget`) and the ephemeral in-process runtime event stream (`createRuntime`/`subscribe`, `@genome/adapter-reference`); canonical demo `SPEC/examples/genome-project.yaml` via `rfc-lifecycle`; nine protected boundaries held; close requires uncached executable conformance **and** a recorded product-acceptance walkthrough (RFC-0009 §14); acceptance floor for this item — WCAG 2.2 AA, browser-first topology behind a compatibility spike, direct package consumption — recorded under "Milestone-1 acceptance floor" below | RFC-0009 (`RFC/0009-phase-4-governed-authoring.md`) | Engineering | Done |
| High | RFC-0010 semantic authoring: the `@genome/authoring` package implementing **`add-agent` only** (department-scoped; intent = required `department` + `id`, optional `role`/`autonomy`/`skills`; a named `applyAddAgent(source, intent)` and no dispatcher; source text in, canonical Genome source out; four failure classes with compiler diagnostics verbatim for `invalid-source`/`invalid-result` and an authoring-owned representation for `invalid-intent`/`conflict`; failure returns no `source` field; three-tier source preservation with `SPEC/examples/genome-project.yaml` as a required fixture; byte-determinism per RFC-0010 §7; authoring never derives `genomeRevision`), **plus the minimal Studio integration required to repeat Milestone-1 product acceptance** (Add agent from the organization surface; the returned source becomes the editor source through the existing ordinary edit lifecycle — no authoring-specific compile path, no direct graph/tree update; source stays visible; WCAG 2.2 AA on both success and failure paths incl. focus behavior). Evidence E1–E19 uncached; eleven protected boundaries held incl. no compiler `Diagnostic`/`CompileStage` change; Checkpoints 1–7 preserved. Close requires a repeated **recorded product-acceptance walkthrough** (RFC-0010 §14) — not a CI gate | RFC-0010 (`RFC/0010-semantic-authoring-operations.md`), ADR-0012 | Engineering | Done |
| High | RFC-0011 `add-workflow` semantic authoring: the `@genome/authoring` **`add-workflow` operation only** (`applyAddWorkflow`; intent = required `id` + `owner` + `steps` with at least one non-empty entry, optional `trigger` restricted to `manual`; source text in, canonical Genome source out; **owner preflight strictly compiler-derived** — original source → `compile` → `runtimeModelTarget(before.graph)` → `agents[].reference` → membership check, resolving nothing independently and never reproducing `buildAgentIndex` (A1); **non-destructive conflict** — refuse `workflows.<id>` already present, returning no source and leaving the existing workflow unchanged, with cross-mapping id collisions accepted and no global uniqueness introduced (A2); RFC-0010's accepted failure classes and source-preservation mechanism reused unchanged; authoring never derives `genomeRevision`), **plus the governance preview** (candidate source compiled through the accepted compiler; `RuntimeWorkflow.governedBy` resolved against `RuntimePolicy.requiresApprovalFrom`; **scoped normatively to the principal Studio actually initiates as**, `STUDIO_OPERATOR` — the preview is not an intrinsic property of the workflow, and changing Studio's initiator returns the preview to the Board (A3); shown **before Create**), **plus the minimal Studio Create-work integration** (start from Engineering; supply id, existing-agent owner and at least one step; understand what governance will apply; create; returned source enters the **existing** editor lifecycle with no authoring-specific compile path; candidate state is preview-only and never replaces canonical projections before Create; run on the **existing** execution surface). Evidence **E1–E43** uncached, including E23 strengthened to prove non-destructive conflict and the six Board additions E38–E43. WCAG 2.2 AA with the programmatically-exposed preview and non-colour-only ungoverned state non-negotiable (A6). Protected boundaries hold: no language, schema, compiler, runtime, event, revision, governance, team, persistence, provider or trigger change; no compiler `Diagnostic`/`CompileStage` change; no `add-policy`. Close requires a **recorded human product-acceptance walkthrough** (RFC-0011 §17, incl. the negative ungoverned-owner case and the question "What caused this work to require approval?") — **not** a CI gate | RFC-0011 (`RFC/0011-add-workflow-authoring-operation.md`), ADR-0012; Board review `docs/reviews/rfc-0011-board-review.md` (Option B, A1–A7 ratified 2026-09-15) | Engineering | Not Started |
| Low | Studio authoring follow-ups (Phase 4 M1 product acceptance, F1–F4): **F1** add a discoverable "View change in source" affordance after successful visual authoring so the canonical Genome source is easier to reach; **F2** clear or invalidate the "Added &lt;agent&gt;" confirmation when a later source edit means that statement no longer describes the current document — **determined from existing source/edit state, never by reparsing YAML in Studio**; **F3** replace ambiguous projection-status wording such as "Everything below was compiled…" with copy that explicitly identifies the projections/state described; **F4** allow Escape to dismiss/cancel the Add agent interaction if consistent with the existing interaction model, restoring focus predictably and making no source change. **Studio-local presentation/interaction only: expands no RFC-0010 semantics** — no new operation, no new intent field, no change to the authoring contract or to canonical-source ownership. Not required for Milestone-1 closure | `docs/reviews/phase-4-m1-product-acceptance.md` §13.7; close review `docs/reviews/phase-4-m1-implementation-close-review.md` §13 | Engineering | Not Started |
